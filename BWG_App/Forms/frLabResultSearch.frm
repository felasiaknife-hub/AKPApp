...
Begin VB.Form frmLabResultSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F049"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frLabResultSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาผลการบันทึกข้อมูล LAB"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "Report No, Analysis No, ชื่อลูกค้า, Waste No, Waste Name"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   2130
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frLabResultSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "frmLabResultSearch"
...
lvResult.ColumnHeaders(3).Text = cboDateType.Text
End Sub
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataLabResult colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataLabResult colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Private Sub Form_Load()
    ...
    With cboDateType
        .AddItem "Report Date"
        .AddItem "Sampling Date"
        .AddItem "Received Date"
        .ListIndex = 0
    End With
End Sub
...
Dim tmpItem As BWGLABResultHeader
    ...
    strCond = "(ReportNo Like '%" & criStr & "%' Or AnalysisNo Like '%" & criStr & "%' Or CompanyName Like '%" & criStr & "%' Or WasteNo Like '%" & criStr & "%' Or WasteName Like '%" & criStr & "%')"
    If cboDateType.ListIndex = 0 Then
        strCond = strCond & " And (ReportDate>='" & dtFrom.ValueYMD & "' And ReportDate<='" & dtTo.ValueYMD & "')"
    ElseIf cboDateType.ListIndex = 1 Then
        strCond = strCond & " And (SamplingDate>='" & dtFrom.ValueYMD & "' And SamplingDate<='" & dtTo.ValueYMD & "')"
    ElseIf cboDateType.ListIndex = 2 Then
        strCond = strCond & " And (ReceivedDate>='" & dtFrom.ValueYMD & "' And ReceivedDate<='" & dtTo.ValueYMD & "')"
    End If
    
'    strCond = SelectWithCompanyID(strCond, True)
    Set colSearchResult_ = tmpS.LabResultHeaderSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.ReportNo)
        tmpList.SubItems(1) = Trim(tmpItem.AnalysisNo)
        If cboDateType.ListIndex = 0 Then
            tmpList.SubItems(2) = FormatYMD_to_DMY(Trim(tmpItem.ReportDate), "/", "/")
        ElseIf cboDateType.ListIndex = 1 Then
            tmpList.SubItems(2) = FormatYMD_to_DMY(Trim(tmpItem.SamplingDate), "/", "/")
        ElseIf cboDateType.ListIndex = 2 Then
            tmpList.SubItems(2) = FormatYMD_to_DMY(Trim(tmpItem.ReceivedDate), "/", "/")
        End If
        tmpList.SubItems(3) = Trim(tmpItem.CustomerName)
        tmpList.SubItems(4) = Trim(tmpItem.WasteNo) & " " & Trim(tmpItem.WasteName)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
