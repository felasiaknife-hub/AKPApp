...
Begin VB.Form frmZoneArea 
   ...
   Begin VB.Image Image2 
      Height          =   705
      Left            =   0
      Picture         =   "frZoneArea.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   6870
   End
End
Attribute VB_Name = "frmZoneArea"
...
Dim tmpStr As String
    ...
    If Trim(txtShortName.Text) <> Trim(curZone_.ZoneName) Or Trim(txtZoneDesc.Text) <> Trim(curZone_.Description) Then
        If Trim(curZone_.ID) = "" Then
            curZone_.ID = GetGUID
            DBConnExc "Insert into tbZoneArea (ZoneID,ZoneName,ZoneDesc) Values ('" & Trim(curZone_.ID) & "','" & (txtShortName.Text) & "','" & Trim(txtZoneDesc.Text) & "')"
        ...
        Else
            DBConnExc "Update tbZoneArea Set ZoneName='" & Trim(txtShortName.Text) & "',ZoneDesc='" & txtZoneDesc.Text & "' WHERE ZoneID='" & curZone_.ID & "'"
...
Dim searchM As New BWGSearchManager
    ...
    End With
    Set colZone_ = searchM.ZoneAreaSearch
    ShowUnitList
    CreateNewZone
End Sub
