...
Begin VB.Form frmStaffSearch 
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   45
      Picture         =   "frStaffSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   5235
      Width           =   915
   ...
   Begin VB.Label Label2 
      ...
      Caption         =   "ชื่อ-นามสกุล, รหัสพนักงาน"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   1740
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frStaffSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7020
   End
...
Public Sub GetStaffDept(ByVal Dept As String)
    StaffDept = Dept
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            If StaffDept = "CR" Then
                callerForm_.SetDataStaffCR colSearchResult_(1)
            Else
                callerForm_.SetDataStaff colSearchResult_(1)
            End If
            Unload Me
        End If
    End If
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
        callerForm_.SetDataStaff colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Dim tmpItem As BWGCompanyStaff
    ...
    strCond = "(StaffFName Like '%" & criStr & "%' Or StaffLName Like '%" & criStr & "%' Or StaffCode Like '%" & criStr & "%')"
    
    Set colSearchResult_ = tmpS.UserSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.StaffFName & " " & tmpItem.StaffLName)
        tmpList.SubItems(1) = Trim(tmpItem.StaffCode)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
