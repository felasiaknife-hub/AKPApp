VERSION 5.00
Begin VB.Form frmComRateManager 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "อัตราค่าคอมมิชชั่นของ Manager - F018"
   ClientHeight    =   6555
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   6210
   ...
   End
End
Attribute VB_Name = "frmComRateManager"
...
Dim x%
    If isChanged_ Then
        DBConnExc "Delete ComRateManager"
...
Private Sub Form_Load()
    ...
    With grComRate.getGridObj
        ...
        .TextMatrix(0, 1) = "ยอดขาย Haz เริ่มต้น (%)"
        .TextMatrix(0, 2) = "ยอดขาย Haz สิ้นสุด (%)"
        .TextMatrix(0, 3) = "โบนัสพิเศษ (บาท)"
    End With
    ShowComRate
End Sub
...
Dim x%, tmpCom As BWGComRateManager
    Set colComRate_ = tmpS.ComRateManagerSearch
    ...
    With grComRate.getGridObj
        ...
        For x = 1 To colComRate_.Count
            ...
            .TextMatrix(x, 1) = Format(tmpCom.HzStartPC, "0.00")
            .TextMatrix(x, 2) = Format(tmpCom.HzEndPC, "0.00")
            .TextMatrix(x, 3) = Format(tmpCom.BonusAmt, "0.00")
            Set tmpCom = Nothing
        Next
    End With
End Sub
...
Dim tmpRate As BWGComRateManager
    With grComRate.getGridObj
        ...
        Else
            Set tmpRate = New BWGComRateManager
            tmpRate.ID = tmpID
    ...
    Select Case col
        Case 1
            If IsNumeric(Trim(tmpVal)) Then
                tmpRate.HzStartPC = Trim(tmpVal)
            Else
                tmpRate.HzStartPC = 0
            End If
        Case 2
            If IsNumeric(Trim(tmpVal)) Then
                tmpRate.HzEndPC = Trim(tmpVal)
            Else
                tmpRate.HzEndPC = 0
            End If
        Case 3
            If IsNumeric(Trim(tmpVal)) Then
                tmpRate.BonusAmt = Trim(tmpVal)
            Else
                tmpRate.BonusAmt = 0
            End If
    End Select
