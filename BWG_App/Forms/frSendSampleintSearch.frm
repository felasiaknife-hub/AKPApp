...
Begin VB.Form frmSendSampleIntSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F121"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frSendSampleintSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frSendSampleintSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7680
   End
End
Attribute VB_Name = "frmSendSampleIntSearch"
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataSendSample colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataSendSample colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpItem As BWGSendSampleHeader
    ...
    strCond = "(SendBillNo Like '%" & criStr & "%' Or CustomerCode Like '%" & criStr & "%' Or CompanyName Like '%" & criStr & "%')"
    If firstActive = False Then
        strCond = strCond & " And (SendDate>='" & dtFrom.ValueYMD & "' And SendDate<='" & dtTo.ValueYMD & "')"
    End If
    strCond = strCond & " AND (ReportNo is not Null) "
    strCond = SelectWithCompanyID(strCond, True)
    Set colSearchResult_ = tmpS.SendSampleHeadSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.SendBillNo)
        tmpList.SubItems(1) = FormatYMD_to_DMY(Trim(tmpItem.SendDate), "/", "/")
        tmpList.SubItems(2) = Trim(tmpItem.ReportNo)
        tmpList.SubItems(3) = Trim(tmpItem.SampleSetName)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
