...
Begin VB.Form frmPrintORD05 
   ...
   End
End
Attribute VB_Name = "frmPrintORD05"
...
Dim m As String
    tmpSql = "Select YearMonth, CustomerID, CompanyID, UnderSaleStaffID, TruckCount, SaleDistrictID, SaleZoneID from vw_PrintOrd05TripCount " & vbCrLf
    tmpSql = tmpSql & "WHERE YearMonth >= '" & cboYear.Text & "/" & Format(cboMonth.ListIndex + 1, "00") & "' And YearMonth<='" & cboEndYear.Text & "/" & Format(cboEndMonth.ListIndex + 1, "00") & "'" & vbCrLf
    ...
    End If
    tmpSql = tmpSql & " AND isCancel <> 'Y'"
    Set tmpRec = GetRS(tmpSql, adOpenForwardOnly, adLockReadOnly)
    If tmpRec Is Nothing Then Exit Sub
    iCount = 1
    If Trim(cboYear.Text) <> Trim(cboEndYear.Text) Then
        RptTypeOrd05 = 1
    Else
        RptTypeOrd05 = 0
    End If
    DBConnExc "DELETE FROM tbPrintOrd05"
    DBConnExc "DELETE FROM tbPrintOrd05 WHERE SessionID='" & sessionID_ & "'"
    DBConnExc "DELETE FROM tbPrintOrd05C"
    DBConnExc "DELETE FROM tbPrintOrd05C WHERE SessionID='" & sessionID_ & "'"
    If RptTypeOrd05 = 0 Then
        Do Until tmpRec.EOF
            DBConnExc "Insert into tbPrintOrd05 (AutoKey, CustomerID, Month" & Trim(Format(Right(Trim(tmpRec("YearMonth")), 2), "##")) & ",SessionID,SaleStaffID,SaleDisctrictID,WorkYear) " & _
            "Values (" & iCount & ",'" & tmpRec("CustomerID") & "'," & tmpRec("TruckCount") & ",'" & sessionID_ & "','" & tmpRec("UnderSaleStaffID") & "','" & tmpRec("SaleDistrictID") & "'," & Left(tmpRec("YearMonth"), 4) & ")"
            tmpRec.MoveNext
            iCount = iCount + 1
        Loop
        tmpRec.Close
        ...
        tmpSql = tmpSql & vbCrLf & " ORDER BY WorkYear,CustomerName,StaffFName,StaffLName, CustomerCode"
    Else
        Do Until tmpRec.EOF
            m = IIf(Left(tmpRec!YearMonth, 4) = Trim(cboYear.Text), Trim(Format(Right(Trim(tmpRec("YearMonth")), 2), "##")), Trim(Format(Right(Trim(tmpRec("YearMonth")), 2) + 12, "##")))
            DBConnExc "Insert into tbPrintOrd05C (AutoKey, CompanyID, CustomerID,Month" & m & ",SessionID,SaleStaffID,SaleDisctrictID,WorkYear) Values (" & _
             iCount & ",'" & tmpRec!CompanyID & "','" & tmpRec("CustomerID") & "'," & tmpRec("TruckCount") & ",'" & sessionID_ & "','" & tmpRec("UnderSaleStaffID") & "','" & tmpRec("SaleDistrictID") & "'," & Left(tmpRec("YearMonth"), 4) & ")"
            tmpRec.MoveNext
            iCount = iCount + 1
        Loop
        tmpRec.Close
        ...
        tmpSql = "SELECT * FROM vw_RptPrintOrd05D " & vbCrLf
        tmpSql = tmpSql & "WHERE SessionID='" & sessionID_ & "' "
    ...
    With rpt1
        ...
        Else
            .ReportFileName = App.Path & "\reports\ORD05D.rpt"
        End If
        .Formulas(0) = "DateRange=" & Chr(34) & "เดือน " & cboMonth.Text & " " & cboYear.Text & " ถึง เดือน " & cboEndMonth.Text & " " & cboEndYear.Text & Chr(34)
...
Private Sub Form_QueryUnload(Cancel As Integer, UnloadMode As Integer)
    DBConnExc "Delete tbPrintOrd05 WHERE SessionID='" & sessionID_ & "'"
End Sub
