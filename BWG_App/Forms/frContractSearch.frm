...
Begin VB.Form frmContractSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F022"
   ClientHeight    =   6375
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   60
      Picture         =   "frContractSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   5550
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาข้อมูลสัญญาการให้บริการ"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   18
         Charset         =   222
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ชื่อลูกค้า, ชื่อบริษัท, เลขที่สัญญา"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   1740
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frContractSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7020
   End
End
Attribute VB_Name = "frmContractSearch"
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataContract colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataContract colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Dim tmpItem As BWGContract
    ...
    strCond = "(DocNo Like '%" & criStr & "%' Or CustomerName Like '%" & criStr & "%' Or CompanyNameTH Like '%" & criStr & "%')"
    Set colSearchResult_ = tmpS.ContractSearch(strCond)
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", FormatYMD_to_DMY(tmpItem.StartDate, "/", "/"))
        tmpList.SubItems(1) = FormatYMD_to_DMY(Trim(tmpItem.EndDate), "/", "/")
