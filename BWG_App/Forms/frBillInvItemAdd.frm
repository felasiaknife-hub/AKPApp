...
Begin VB.Form frmReceiptInvItemAdd 
   BackColor       =   &H00FFFFFF&
   ...
   Begin VB.Label Label2 
      ...
      Caption         =   "เลขที่ใบวางบิล, เลขที่ Manifest, คำอธิบาย"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   1
      Left            =   1800
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "เลือกรายการสำหรับ เพิ่ม/ลด หนี้"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   18
         Charset         =   222
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   0
      Picture         =   "frBillInvItemAdd.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11400
   End
End
Attribute VB_Name = "frmReceiptInvItemAdd"
...
Dim tmpKey As String
    ...
    For x = 1 To colItemDel_.Count
        tmpKey = Trim("'" & colItemDel_(x).BillingHeaderID & "_" & colItemDel_(x).ManifestID & "_" & colItemDel_(x).JobDetID & "'")
        If isExist(colOriSelProduct_, tmpKey) Then
            callerFrm_.RemoveBillData colItemDel_(x).BillingHeaderID, colItemDel_(x).ManifestID, colItemDel_(x).JobDetID
        End If
    Next
...
Dim tmpBill As BWGAdjDeptDetail
Dim tmpArr
...
Dim x%
    ...
    sqlStr = "(BillingNo Like '%" & criStr1 & "%' Or DocNo Like '%" & criStr1 & "%' Or WasteName Like '%" & criStr1 & "%') And CompanyID='" & companyID_ & "'"
    If Not curCustomer_ Is Nothing Then
        sqlStr = sqlStr & " And CustomerID='" & curCustomer_.ID & "'"
    End If
    
    Set colSearchResult_ = tmpS.AdjustDeptItemSearch(sqlStr)
    lvResult.ListItems.Clear
    For x = 1 To colSearchResult_.Count
        Set tmpBill = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpBill.BillingHeaderID & "_" & tmpBill.ManifestID & "_" & tmpBill.JobDetID & "'", tmpBill.BillingNo)
        tmpList.SubItems(1) = tmpBill.MenifestDocNo
        tmpList.SubItems(2) = tmpBill.AdjBillDesc
...
Public Sub SetCallerForm(frm As Form, selCust As BWGCustomer, selCompanyID As String)
Dim x%
Dim tmpBill As BWGAdjDeptDetail
Dim tmpList As ListItem
...
Dim strCond As String
    ...
    If Not colOriSelProduct_ Is Nothing Then
        For x = 1 To colOriSelProduct_.Count
            Set tmpBill = colOriSelProduct_(x)
            Set tmpList = lvSelected.ListItems.Add(, "'" & tmpBill.BillingHeaderID & "_" & tmpBill.ManifestID & "_" & tmpBill.JobDetID & "'", tmpBill.BillingNo)
            tmpList.SubItems(1) = tmpBill.MenifestDocNo
            tmpList.SubItems(2) = tmpBill.AdjBillDesc
            Set tmpList = Nothing
            Set tmpBill = Nothing
