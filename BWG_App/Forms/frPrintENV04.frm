...
Begin VB.Form frmPrintENV04 
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "รายงานวันใบอนุญาติหมดอายุ (ENV04)"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Image Image2 
      ...
      Index           =   1
      Left            =   0
      Picture         =   "frPrintENV04.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11280
   End
End
Attribute VB_Name = "frmPrintENV04"
...
Dim tmpSql As String
    With rpt1
        ...
        .ReportFileName = App.Path & "\reports\ENV04.rpt"
        tmpSql = ""
        tmpSql = "SELECT * FROM vw_RptENV04 " & vbCrLf
        
        tmpSql = tmpSql & "WHERE LicenseExpDate>='" & dtFrom.ValueYMD & "' And LicenseExpDate<='" & dtTo.ValueYMD & "' "
        If Not curStaff_ Is Nothing Then
            If Trim(curStaff_.ID) <> "" Then
                tmpSql = tmpSql & " And SaleStaffID='" & curStaff_.ID & "' "
            End If
        End If
        ...
        If Trim(txtCustomer.Text) <> "" Then
            If Trim(curCust_.ID) <> "" Then
                tmpSql = tmpSql & " And CustomerID='" & curCust_.ID & "' "
            End If
        End If
        tmpSql = tmpSql & vbCrLf & " ORDER BY CustomerName,WasteName"
        .Formulas(0) = "DateRange=" & Chr(34) & "ใบอนุญาตหมดอายุในช่วงวันที่ " & dtFrom.ValueDMY & " ถึงวันที่ " & dtTo.ValueDMY & Chr(34)
...
Private Sub txtCustomer_Validate(Cancel As Boolean)
    If curCust_ Is Nothing Then Set curCust_ = New BWGCustomer
    If Trim(txtCustomer.Text) = "" And Trim(curCust_.CustomerName) <> Trim(txtCustomer.Text) Then
        Set curCust_ = Nothing
        Set curCust_ = New BWGCustomer
    Else
        If Trim(curCust_.CustomerName) <> Trim(txtCustomer.Text) Then
            Set curCust_ = Nothing
            Set curCust_ = New BWGCustomer
            ...
            With frmCustomerSearch
                ...
                .Show 1
            End With
            If curCust_.ID = "" Then txtCustomer.Text = ""
        End If
    End If
End Sub
...
Private Sub txtStaff_Validate(Cancel As Boolean)
    ...
    If Trim(txtStaff.Text) = "" And Trim(curStaff_.StaffFName & " " & curStaff_.StaffLName) <> Trim(txtStaff.Text) Then
        Set curStaff_ = Nothing
        Set curStaff_ = New BWGCompanyStaff
    Else
        If Trim(curStaff_.StaffFName & " " & curStaff_.StaffLName) <> Trim(txtStaff.Text) Then
            Set curStaff_ = Nothing
            Set curStaff_ = New BWGCompanyStaff
            Load frmStaffSearch
            With frmStaffSearch
                ...
                .Show 1
            End With
            If curStaff_.ID = "" Then txtStaff.Text = ""
        End If
    End If
End Sub
...
Dim x%
    Set curCust_ = Nothing
    Set curCust_ = selCust
    If curCust_ Is Nothing Then Exit Sub
    With curCust_
