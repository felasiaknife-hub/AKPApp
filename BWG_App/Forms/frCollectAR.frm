...
Dim tmpGrid As MSFlexGrid, curTripID$, selResult%, rowPerPage%, x%, detailCount%, curRow%, curRecNo%, temp1%, temp2%, temp3%, temp4%
...
Dim tmpTrip As BWGJobDataTimeTable, tmpSite As BWGCustomerBillAddr, tmpList As ListItem
    ...
    With rpt1
                    ...
                    If Not tmpList Is Nothing Then
                        If tmpList.Checked Then
                            ...
                            If tmpTrip.TripEquipFee + tmpTrip.TripOtherFee + tmpTrip.TripTranFee > 0 Then
                                ...
                                .Formulas(20 + (curRow - 1) * 4) = "Col1_" & curRow & "=" & Chr(34) & "ค่าขนส่งรถ  " & tmpList.SubItems(1) & "  " & tmpList.SubItems(4) & Chr(34)
                                ...
                                PrintNow rpt1, curRow, rowPerPage, curRecNo, detailCount
                            End If
                            If tmpTrip.IsDiscountPriceTrans Then
                                .Formulas(20 + (curRow - 1) * 4) = "Col1_" & curRow & "=" & Chr(34) & "ส่วนลด ค่าขนส่งรถ  " & tmpList.SubItems(1) & Chr(34)
                                .Formulas(21 + (curRow - 1) * 4) = "Col2_" & curRow & "=" & Chr(34) & "1 เที่ยว" & Chr(34)
                                .Formulas(22 + (curRow - 1) * 4) = "Col3_" & curRow & "=" & Chr(34) & Format((tmpTrip.TripEquipFee + tmpTrip.TripOtherFee + tmpTrip.TripTranFee) * -1, "#,##0.00") & " บาท/เที่ยว" & Chr(34)
                                .Formulas(23 + (curRow - 1) * 4) = "Col4_" & curRow & "=" & Chr(34) & Format((tmpTrip.TripEquipFee + tmpTrip.TripOtherFee + tmpTrip.TripTranFee) * -1, "#,##0.00") & Chr(34)
                                curRow = curRow + 1
                                PrintNow rpt1, curRow, rowPerPage, curRecNo, detailCount
                            End If
                            If tmpTrip.TripMoreWorker > 0 Then
                                '.Formulas(20 + (curRow - 1) * 4) = "Col1_" & curRow & "=" & Chr(34) & "ค่าแรงคนงาน " & tmpTrip.CarRegisNo & Chr(34)
                                .Formulas(20 + (curRow - 1) * 4) = "Col1_" & curRow & "=" & Chr(34) & "ค่าแรงคนงาน " & Chr(34)
        ...
        Next
            ...
            End If
                    curRow = curRow + 1
                    .Formulas(20 + (curRow - 1) * 4) = "Col1_" & curRow & "=" & Chr(34) & "ค่าบริการวิเคราะห์" & Chr(34)
                    .Formulas(21 + (curRow - 1) * 4) = "Col2_" & curRow & "=" & Chr(34) & "" & Chr(34)
                    .Formulas(22 + (curRow - 1) * 4) = "Col3_" & curRow & "=" & Chr(34) & "" & Chr(34)
                    .Formulas(23 + (curRow - 1) * 4) = "Col4_" & curRow & "=" & Chr(34) & "0.00" & Chr(34)
                    curRow = curRow + 1
                     curRecNo = curRecNo + 1 '4
                    PrintNow rpt1, curRow, rowPerPage, curRecNo, detailCount
...
Dim x%, tmpAmt As Double
    ...
    For x = 1 To lvTrip.ListItems.Count
        If lvTrip.ListItems(x).Checked And IsNumeric(lvTrip.ListItems(x).SubItems(2)) Then
            tmpAmt = tmpAmt + CDbl(lvTrip.ListItems(x).SubItems(2)) + CDbl(lvTrip.ListItems(x).SubItems(3))
        End If
    Next
    ...
    For x = 1 To colTrip.Count '******** วบเช็ค Waste วนตามใบคุม, ใบเมนิเฟส, ประเภทเวสต์
        ...
        Else
            ...
            Else
                ...
                tmpWaste.WasteName = " " & tmpJobDet.WasteEUCode & "   " & tmpJobDet.WasteName
...
'        Set tmpCol2 = myTripControl_.GetMenifests("A")
        ...
        For x = 1 To tmpCol2.Count
            ...
            If Trim(tmpItem.id) <> Trim(curMfID) Then
                ...
                .TextMatrix(curRow, 1) = "วันที่ " & FormatYMD_to_DMY(tmpItem.WorkDate, "/", "/")
                curRow = curRow + 1
                   If .Rows - 1 = curRow Then .Rows = .Rows + 5
                    .TextMatrix(curRow, 7) = tmpItem.id
                    .TextMatrix(curRow, 0) = tmpItem.JobDataCarID & "_" & tmpItem.TripNo
                If tmpItem.RefME <> "" Then
                    .TextMatrix(curRow, 1) = "เลขที่" & tmpItem.DocNo & "/" & tmpItem.DocNoRef & " อ้างอิง " & tmpItem.RefME
                Else
                    .TextMatrix(curRow, 1) = "เลขที่ " & tmpItem.DocNo & "/" & tmpItem.DocNoRef
                End If
                .TextMatrix(curRow, 2) = ""
                .TextMatrix(curRow, 3) = ""
                .TextMatrix(curRow, 4) = ""
...
'                If curTreatUnitID = "" Then curTreatUnitID = Trim(tmpJobDet.JobTreatmentUnitID)
                ...
                If Trim(tmpJobDet.FailType) = "" Then
                    .TextMatrix(curRow, 7) = tmpItem.id
                    .TextMatrix(curRow, 0) = tmpItem.JobDataCarID & "_" & tmpItem.TripNo
                    .TextMatrix(curRow, 1) = " " & tmpJobDet.WasteEUCode & "   " & tmpJobDet.WasteName
                    ...
                    Else
                        ...
                        If UCase(tmpJobDet.WasteType) <> "NON" And UCase(tmpJobDet.WasteType) <> "NBL" Then
                            If tmpJobDet.ChargeWeightType = "A" Then
                                If InStr(1, tmpJobDet.JobTreatmentUnitID, "กิโลกรัม") Then
...
Dim tmpDetail As New BWGBillingDetail
    ...
    With selItem
        ...
        tmpTrip.TruckTypeDesc = .TruckSubTypeName
        ...
        tmpTrip.IsDiscountPriceTrans = .IsDiscountPriceTrans
        tmpTrip.AumphurName = .AumphurName
        tmpTrip.ProvinceName = .ProvinceName
        End With
...
Dim x%, tmpTrip As BWGJobDataTimeTable, y%
    ...
    For x = 1 To myBillHeader_.TripControls.Count
        ...
        If lvTrip.FindItem(tmpTrip.TimeTableNo) Is Nothing Then
            ...
            tmpList.SubItems(1) = tmpTrip.TruckTypeDesc
            tmpList.SubItems(4) = tmpTrip.AumphurName & "/" & tmpTrip.ProvinceName
            If Trim(tmpTrip.CustBillID) <> "" And Trim(tmpTrip.CustBillID) <> Trim(myBillHeader_.id) Then
                ...
                If tmpTrip.IsDiscountPriceTrans Then
                    tmpList.SubItems(3) = Format((tmpTrip.TripAllFee) * -1, "#,##0.00") & " (Billed)"
                Else
                    tmpList.SubItems(3) = Format(0, "#,##0.00")
                End If
            ...
            Else
                tmpList.SubItems(2) = Format(tmpTrip.TripEquipFee + tmpTrip.TripMoreWorkerFee + tmpTrip.TripOtherFee + tmpTrip.TripTranFee, "#,##0.00")
                  If tmpTrip.IsDiscountPriceTrans Then
                    tmpList.SubItems(3) = Format((tmpTrip.TripTranFee) * -1, "#,##0.00")
                Else
                    tmpList.SubItems(3) = Format(0, "#,##0.00")
                End If
        ...
        Else
            ...
            If tmpTrip.isMain Then
                tmpList.SubItems(1) = tmpTrip.TruckSubTypeDesc '& ", " & tmpList.SubItems(1)
            Else
                tmpList.SubItems(1) = tmpTrip.TruckSubTypeDesc 'tmpList.SubItems(1) & ", " & tmpTrip.CarRegisNo
            End If
        End If
        Set tmpList = Nothing
        Set tmpTrip = Nothing
