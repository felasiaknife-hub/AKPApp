...
Begin VB.Form frmPrintCR03Search 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F069"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frPrintCR03Search.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาใบแจ้งการกำจัดและบำบัดกาก"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "เลขที่ใบแจ้ง, ชื่อ-รหัสบริษัทที่ออกใบแจ้ง, ชื่อลูกค้า"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   2130
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frPrintCR03Search.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "frmPrintCR03Search"
...
Private Sub cmdOpen_Click()
    If lvResult.ListItems.Count = 0 Then
        MsgBox "ไม่มีข้อมูลใบแจ้งที่ต้องการ", vbExclamation
        Exit Sub
    End If
    If lvResult.SelectedItem Is Nothing Then
        MsgBox "กรุณาเลือกข้อมูลใบแจ้งที่ต้องการ", vbExclamation
        Exit Sub
    End If
    callerForm_.SetDataCR03 colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataCR03 colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpItem As BWGPrintCR03
    ...
    strCond = "(CR03No Like '%" & criStr & "%' Or CompanyNameTH Like '%" & criStr & "%' Or CompanyCode Like '%" & criStr & "%' Or CustomerName Like '%" & criStr & "%')"
    If firstActive = False Then
        strCond = strCond & " And (IssuedDate>='" & dtFrom.ValueYMD & "' And IssuedDate<='" & dtTo.ValueYMD & "')"
    End If
    Set colSearchResult_ = tmpS.CR03HeaderSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.CR03No)
        tmpList.SubItems(1) = FormatYMD_to_DMY(Trim(tmpItem.IssuedDate), "/", "/")
        tmpList.SubItems(2) = Trim(tmpItem.CompanyName)
