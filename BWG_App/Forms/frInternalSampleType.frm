...
Begin VB.Form frmInternalSampleType 
   BackColor       =   &H00FFFFFF&
   ...
   Begin VB.Label Label1 
      ...
      Caption         =   "ประเภทตัวอย่างสำหรับการตรวจสอบภายใน"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Image Image2 
      Height          =   705
      Left            =   0
      Picture         =   "frInternalSampleType.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   6870
   End
End
Attribute VB_Name = "frmInternalSampleType"
...
Dim curSampleType_ As BWGInternalSampleType
...
Dim aa
    If Trim(txtShortName.Text) <> Trim(curSampleType_.TypeName) Then
...
Dim tmpStr As String
    ...
    If curSampleType_ Is Nothing Then Exit Sub
    ...
    If Trim(txtShortName.Text) <> Trim(curSampleType_.TypeName) Then
        If Trim(curSampleType_.ID) = "" Then
            curSampleType_.ID = GetGUID
            DBConnExc "Insert into tbInternalSampleType (IntSampleTypeID,SampleTypeName) Values ('" & Trim(curSampleType_.ID) & "','" & (txtShortName.Text) & "')"
            Set tmpList = lvList.ListItems.Add(, "'" & curSampleType_.ID & "'", txtShortName.Text)
            curSampleType_.TypeName = Trim(txtShortName.Text)
            colSampleType_.Add curSampleType_, "'" & curSampleType_.ID & "'"
        Else
            DBConnExc "Update tbInternalSampleType Set SampleTypeName='" & Trim(txtShortName.Text) & "' WHERE IntSampleTypeID='" & curSampleType_.ID & "'"
            Set tmpList = lvList.ListItems("'" & curSampleType_.ID & "'")
            curSampleType_.TypeName = Trim(txtShortName.Text)
        End If
...
Dim searchM As New BWGSearchManager
    ...
    End With
    Set colSampleType_ = searchM.InternalSampleTypeSearch
    ShowSampleTypeList
    CreateNewSampleType
End Sub

Private Sub ShowSampleTypeList()
Dim x%
Dim tmpList As ListItem
Dim tmpItem As BWGInternalSampleType
    If colSampleType_ Is Nothing Then Exit Sub
    lvList.ListItems.Clear
    For x = 1 To colSampleType_.Count
        Set tmpItem = colSampleType_(x)
...
On Error Resume Next
    If Not callerForm_ Is Nothing And Not lvList.SelectedItem Is Nothing Then
        callerForm_.SetDataSampleType colSampleType_(lvList.SelectedItem.Key)
        Unload Me
    End If
End Sub
...
Dim tmpKey As String
    ...
    Set curSampleType_ = Nothing
    tmpKey = Trim(lvList.SelectedItem.Key)
    If isExist(colSampleType_, tmpKey) Then
        Set curSampleType_ = colSampleType_(tmpKey)
        txtShortName.Text = curSampleType_.TypeName
        curRole_ = UpdateRole
    End If
End Sub

Private Sub CreateNewSampleType()
    Set curSampleType_ = New BWGInternalSampleType
    curSampleType_.ID = ""
    curRole_ = AddNewRole
End Sub
