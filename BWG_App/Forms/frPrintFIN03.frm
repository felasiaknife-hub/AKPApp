...
Begin VB.Form frmPrintFIN03 
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "พิมพ์รายงานสรุปยอดใบสำคัญจ่าย"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Image Image2 
      ...
      Index           =   1
      Left            =   0
      Picture         =   "frPrintFIN03.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11280
   End
End
Attribute VB_Name = "frmPrintFIN03"
...
Dim tmpSql As String
    With rpt1
        ...
        .ReportFileName = App.Path & "\reports\FIN03.rpt"
        tmpSql = ""
        tmpSql = "SELECT * FROM vw_RptFIN03 " & vbCrLf
        tmpSql = tmpSql & "WHERE BillPayDate>='" & dtFrom.ValueYMD & "' And BillPayDate<='" & dtTo.ValueYMD & "'"
        ...
        If Not curVendor_ Is Nothing Then
            If Trim(curVendor_.ID) <> "" Then
                tmpSql = tmpSql & " And SupplierID='" & curVendor_.ID & "' "
            End If
        End If
        tmpSql = tmpSql & vbCrLf & " ORDER BY BillPayNo"
...
Private Sub txtVendor_Validate(Cancel As Boolean)
    ...
    If Trim(txtVendor.Text) = "" And Trim(curVendor_.NameTH) <> Trim(txtVendor.Text) Then
        Set curVendor_ = Nothing
        Set curVendor_ = New BWGTransportVendor
    Else
        If Trim(curVendor_.NameTH) <> Trim(txtVendor.Text) Then
            Set curVendor_ = Nothing
            Set curVendor_ = New BWGTransportVendor
            ...
            With frmTransportVenderSearch
                ...
                .Show 1
            End With
            If curVendor_.ID = "" Then txtVendor.Text = ""
        End If
    End If
End Sub
