...
Option Explicit
Dim curUnit_ As BWGGeneralUnit
...
Dim tmpStr As String
    ...
    If Trim(txtShortName.Text) <> Trim(curUnit_.UnitName) Then
        If Trim(curUnit_.ID) = "" Then
            curUnit_.ID = GetGUID
            DBConnExc "Insert into tbGeneralUnit (GenUnitID,UnitName) Values ('" & Trim(curUnit_.ID) & "','" & (txtShortName.Text) & "')"
        ...
        Else
            DBConnExc "Update tbGeneralUnit Set UnitName='" & Trim(txtShortName.Text) & "' WHERE GenUnitId='" & curUnit_.ID & "'"
...
Dim searchM As New BWGSearchManager
    ...
    End With
    Set colUnit_ = searchM.UnitGeneralSearch
    ShowUnitList
    CreateNewUnit
End Sub
...
Private Sub CreateNewUnit()
    Set curUnit_ = New BWGGeneralUnit
    curUnit_.ID = ""
    curRole_ = AddNewRole
End Sub
