...
Begin VB.Form frmPrintORD04 
   ...
   Begin VB.Image Image2 
      ...
      Index           =   1
      Left            =   0
      Picture         =   "frPrintORD04.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11280
   End
End
Attribute VB_Name = "frmPrintORD04"
...
Dim tmpSql As String, iCount As Long
    With rpt1
        ...
        .ReportFileName = App.Path & "\reports\ORD04.rpt"
        tmpSql = ""
        tmpSql = "SELECT * FROM vw_RptOrd04 " & vbCrLf
        
        tmpSql = tmpSql & "WHERE ((FirstWOrkDate is null And LastWorkDate is null) Or (LastWorkDate<'" & cboYear.Text & "/" & Format((cboMonth.ListIndex + 1), "00") & "/01') Or (LastWorkDate>'" & cboEndYear.Text & "/" & Format((cboEndMonth.ListIndex + 1), "00") & "/31'))"
        ...
        tmpSql = tmpSql & vbCrLf & " ORDER BY CompanyName"
        .Formulas(0) = "DateRange=" & Chr(34) & cboMonth.Text & " พ.ศ. " & cboYear.Text & "  ถึง  " & cboEndMonth.Text & " พ.ศ. " & cboEndYear.Text & Chr(34)
