...
Begin VB.Form frmPaytoDeptorSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F065"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frPaytoDeptorSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหารายการชำระเงินเจ้าหนี้"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "เลขที่ใบจ่าย, ชื่อเจ้าหนี้, รหัสเจ้าหนี้"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   2130
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frPaytoDeptorSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "frmPaytoDeptorSearch"
...
Dim PaymentTypeSearch As String
Public Sub PaymentType(ByVal PayType As String)
    PaymentTypeSearch = PayType
End Sub

Public Sub getCriteria(ByVal criStr As String)
    firstActive = True
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataPaytoDeptor colSearchResult_(lvResult.SelectedItem.Key)
    Me.Hide
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataPaytoDeptor colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpS As BWGSearchManager
    ...
    If PaymentTypeSearch <> "PaymentVoucher" Then
        Dim tmpItem As BWGBillPayHeader
        strCond = "(BillPayNo Like '%" & criStr & "%' Or VendorCode Like '%" & criStr & "%' Or VendorName Like '%" & criStr & "%')"
        If firstActive = False Then
            strCond = strCond & " And (BillPayDate>='" & dtFrom.ValueYMD & "' And BillPayDate<='" & dtTo.ValueYMD & "')"
        End If
        strCond = SelectWithCompanyID(strCond, True)
        
        Set colSearchResult_ = tmpS.BillPayHeaderSearch(strCond)
        Set tmpS = Nothing
        For x = 1 To colSearchResult_.Count
            Set tmpItem = colSearchResult_(x)
            Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.BillPayNo)
            tmpList.SubItems(1) = FormatYMD_to_DMY(Trim(tmpItem.BillPayDate), "/", "/")
            tmpList.SubItems(2) = Trim(tmpItem.SupplierName)
            If tmpItem.isCancel Then
                ...
                Next
            Else
                tmpList.SubItems(3) = "Active"
            End If
            Set tmpItem = Nothing
            Set tmpList = Nothing
        Next
    Else
        Dim ItemPayment As BWGPaymentHeader
        strCond = "(PaymentNo Like '%" & criStr & "%')"
'        strCond = "(PaymentNo Like '%" & criStr & "%' Or VendorCode Like '%" & criStr & "%' Or VendorName Like '%" & criStr & "%')"
        If firstActive = False Then
            strCond = strCond & " And (PaymentDate >= '" & dtFrom.ValueYMD & "' And PaymentDate <= '" & dtTo.ValueYMD & "')"
        End If
        strCond = SelectWithCompanyID(strCond, True)
        
        Set colSearchResult_ = tmpS.PaymentHeaderSearch(strCond)
        Set tmpS = Nothing
        For x = 1 To colSearchResult_.Count
            Set ItemPayment = colSearchResult_(x)
            Set tmpList = lvResult.ListItems.Add(, "'" & ItemPayment.ID & "'", ItemPayment.PaymentNo)
            tmpList.SubItems(1) = FormatYMD_to_DMY(Trim(ItemPayment.PaymentDate), "/", "/")
            tmpList.SubItems(2) = Trim(ItemPayment.SupplierName)
            If ItemPayment.isCancel Then
                tmpList.SubItems(3) = "ยกเลิก"
                tmpList.ForeColor = vbRed
                tmpList.Bold = True
                For Y = 1 To 3
