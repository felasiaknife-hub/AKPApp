...
Begin VB.Form frmPrintCR03WasteAdd 
   BackColor       =   &H00FFFFFF&
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   0
      Picture         =   "frPrintCR03WasteAdd.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11400
   End
End
Attribute VB_Name = "frmPrintCR03WasteAdd"
...
Dim tmpKey As String
    ...
    For x = 1 To colItemAdd_.Count
        ...
        tmpKey = Trim("'" & colItemAdd_(x).ID & "_" & colItemAdd_(x).menifestID & "'")
...
Dim x%
    ...
    sqlStr = "(WasteNo Like '%" & criStr1 & "%' Or WasteName Like '%" & criStr1 & "%' Or DocNo Like '%" & criStr1 & "%' Or CarRegisID Like '%" & criStr1 & "%') And WorkDate>='" & dtFrom.ValueYMD & "' And WorkDate<='" & dtTo.ValueYMD & "' And (CR03ID='" & refID_ & "' Or CR03ID is null)"
    ...
    End If
    Set colSearchResult_ = tmpS.CR03AddWasteSearch(sqlStr)
    lvResult.ListItems.Clear
    For x = 1 To colSearchResult_.Count
        Set tmpJobDet = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpJobDet.ID & "_" & tmpJobDet.menifestID & "'", FormatYMD_to_DMY(tmpJobDet.MenifestWorkDate))
        tmpList.SubItems(1) = tmpJobDet.MenifestDocNo
        tmpList.SubItems(2) = tmpJobDet.WasteName
        tmpList.SubItems(3) = tmpJobDet.WasteEUCode
        tmpList.SubItems(4) = tmpJobDet.MenifestCarRegisID
        If tmpJobDet.ChargeWeightType = "A" Then
            tmpList.SubItems(5) = tmpJobDet.DisposerWeight
        ElseIf tmpJobDet.ChargeWeightType = "B" Then
            tmpList.SubItems(5) = tmpJobDet.EstWasteQty
        End If
...
Public Sub SetCallerForm(frm As Form, selCompany As BWGCompanyConfig, selCust As BWGCustomer)
Dim x%
Dim tmpJobDet As BWGJobDataDetail
Dim tmpList As ListItem
...
Dim strCond As String
    ...
    If Not colOriSelProduct_ Is Nothing Then
        For x = 1 To colOriSelProduct_.Count
            ...
            Set tmpJobDet = colOriSelProduct_(x)
            Set tmpList = lvSelected.ListItems.Add(, "'" & tmpJobDet.ID & "_" & tmpJobDet.menifestID & "'", FormatYMD_to_DMY(tmpJobDet.MenifestWorkDate))
            tmpList.SubItems(1) = tmpJobDet.MenifestDocNo
            tmpList.SubItems(2) = tmpJobDet.WasteName
            tmpList.SubItems(3) = tmpJobDet.WasteEUCode
            tmpList.SubItems(4) = tmpJobDet.MenifestCarRegisID
