...
Begin VB.Form frmPrintFIN05 
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "พิมพ์รายงานภาษีขาย (FIN05)"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Image Image2 
      ...
      Index           =   1
      Left            =   0
      Picture         =   "frPrintFIN05.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11280
   End
End
Attribute VB_Name = "frmPrintFIN05"
...
Dim tmpSql As String
    With rpt1
        ...
        .ReportFileName = App.Path & "\reports\FIN05.rpt"
        tmpSql = ""
        tmpSql = "SELECT * FROM vw_RptFIN05 " & vbCrLf
        tmpSql = tmpSql & "WHERE InvDate>='" & dtFrom.ValueYMD & "' And InvDate<='" & dtTo.ValueYMD & "'"
        If cboCompany.ListCount > 1 Then
            If cboCompany.ListIndex > 0 Then
                tmpSql = tmpSql & " AND InvCompanyID='" & CurrentUser.WorkCompanies(cboCompany.ListIndex + cboIndex).ID & "' "
            End If
        Else
            If cboCompany.ListIndex >= 0 Then
                tmpSql = tmpSql & " AND InvCompanyID='" & CurrentUser.WorkCompanies(cboCompany.ListIndex + cboIndex).ID & "' "
        ...
        If Not curCustomer_ Is Nothing Then
            ...
            End If
        End If
        tmpSql = tmpSql & vbCrLf & " ORDER BY InvNo"
        .Formulas(0) = "ShowCompanyName=" & Chr(34) & cboCompany.Text & Chr(34)
        If cboCompany.ListCount > 1 Then
            If cboCompany.ListIndex > 0 Then
                .Formulas(1) = "ShowTaxID=" & Chr(34) & CurrentUser.WorkCompanies(cboCompany.ListIndex + cboIndex).TaxID & Chr(34)
            End If
        Else
            .Formulas(1) = "ShowTaxID=" & Chr(34) & CurrentUser.WorkCompanies(cboCompany.ListIndex + cboIndex).TaxID & Chr(34)
        End If
        .Formulas(2) = "ShowMonth=" & Chr(34) & dtFrom.MonthNameShort & " " & dtFrom.YearValue & Chr(34)
        .SQLQuery = tmpSql
