...
Begin VB.Form frmFuelBuyRecordSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F035"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frFuelBuyRecordSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาข้อมูลการซื้อน้ำมัน"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ทะเบียนรถ, Vendor, ชื่อคนขับ"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   2130
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frFuelBuyRecordSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "frmFuelBuyRecordSearch"
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataBuyRecord colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataBuyRecord colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpItem As BWGFuelBuyRecord
    ...
    strCond = "(DriverName Like '%" & criStr & "%' Or CarRegisID Like '%" & criStr & "%' Or NameTH Like '%" & criStr & "%')"
    Set colSearchResult_ = tmpS.FuelBuyRecordSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", FormatYMD_to_DMY(tmpItem.BuyDate, "/", "/"))
        tmpList.SubItems(1) = Trim(tmpItem.CarRegisID)
        tmpList.SubItems(2) = Trim(tmpItem.CarTypeDesc)
        tmpList.SubItems(3) = Trim(tmpItem.VendorName)
        tmpList.SubItems(4) = Trim(tmpItem.DriverName)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
    ...
    If lvResult.ListItems.Count = 0 Then
        ...
        If result = vbYes Then
            frmFuelBuyRecord.cmdSearch.Enabled = False
