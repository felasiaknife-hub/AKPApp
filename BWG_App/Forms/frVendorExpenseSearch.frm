...
Begin VB.Form frmVendorExpenseSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F147"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frVendorExpenseSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาข้อมูลรายจ่ายอื่นๆ"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frVendorExpenseSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "frmVendorExpenseSearch"
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataVendorExpense colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataVendorExpense colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpItem As BWGVendorOtherExpense
    ...
    strCond = "(NameTH Like '%" & criStr & "%' Or BillDesc Like '%" & criStr & "%')"
    Set colSearchResult_ = tmpS.VendorExpenseSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", FormatYMD_to_DMY(tmpItem.ExpenseDate, "/", "/"))
