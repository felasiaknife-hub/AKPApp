VERSION 5.00
Object = "{00025600-0000-0000-C000-000000000046}#5.2#0"; "Crystl32.OCX"
Begin VB.Form frmPrintFIN14 
   ClientHeight    =   4785
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   6255
   LinkTopic       =   "Form1"
   ScaleHeight     =   4785
   ScaleWidth      =   6255
   StartUpPosition =   2  'CenterScreen
   Begin VB.CommandButton cmdPreview 
      Caption         =   "Preview"
      Height          =   795
      Left            =   3360
      Style           =   1  'Graphical
      TabIndex        =   11
      Top             =   3840
      Width           =   915
   End
   Begin VB.CommandButton cmdClose 
      Caption         =   "ปิด"
      Height          =   795
      Left            =   5280
      Style           =   1  'Graphical
      TabIndex        =   10
      Top             =   3840
      Width           =   915
   End
   Begin VB.CommandButton cmdPrint 
      Caption         =   "พิมพ์"
      Height          =   795
      Left            =   4320
      Style           =   1  'Graphical
      TabIndex        =   9
      Top             =   3840
      Width           =   915
   End
   Begin VB.Frame Frame1 
      Appearance      =   0  'Flat
      BackColor       =   &H80000005&
      ForeColor       =   &H80000008&
      Height          =   2835
      Left            =   180
      TabIndex        =   0
      Top             =   960
      Width           =   6015
      Begin VB.ComboBox cboCheck 
         Height          =   315
         ItemData        =   "frmPrintFIN14.frx":0000
         Left            =   1200
         List            =   "frmPrintFIN14.frx":0013
         TabIndex        =   17
         Top             =   2220
         Width           =   2355
      End
      Begin VB.ComboBox cboRE_Men 
         Height          =   315
         Left            =   1200
         TabIndex        =   14
         Top             =   1800
         Width           =   2355
      End
      Begin VB.OptionButton OpChek2 
         BackColor       =   &H80000009&
         Height          =   315
         Left            =   2580
         TabIndex        =   13
         Top             =   660
         Width           =   195
      End
      Begin VB.OptionButton OptChek1 
         BackColor       =   &H80000009&
         Height          =   255
         Left            =   1200
         TabIndex        =   12
         Top             =   660
         Value           =   -1  'True
         Width           =   195
      End
      Begin VB.ComboBox cboCompany 
         Height          =   315
         Left            =   1200
         Style           =   2  'Dropdown List
         TabIndex        =   2
         Top             =   1020
         Width           =   4395
      End
      Begin VB.TextBox txtCustomer 
         Appearance      =   0  'Flat
         BackColor       =   &H00FF8080&
         ForeColor       =   &H00FFFFFF&
         Height          =   315
         Left            =   1200
         TabIndex        =   1
         Top             =   1440
         Width           =   4365
      End
      Begin WasteManagment.ctlDate dtFrom 
         Height          =   315
         Left            =   1200
         TabIndex        =   3
         Top             =   240
         Width           =   1755
         _ExtentX        =   3096
         _ExtentY        =   556
         BackColor       =   -2147483633
      End
      Begin WasteManagment.ctlDate dtTo 
         Height          =   315
         Left            =   3660
         TabIndex        =   4
         Top             =   240
         Width           =   1755
         _ExtentX        =   3096
         _ExtentY        =   556
         BackColor       =   -2147483633
      End
      Begin Crystal.CrystalReport rpt1 
         Left            =   60
         Top             =   120
         _ExtentX        =   741
         _ExtentY        =   741
         _Version        =   348160
         WindowState     =   2
         PrintFileLinesPerPage=   60
         WindowShowPrintSetupBtn=   -1  'True
      End
      Begin VB.Label Label6 
         BackColor       =   &H80000009&
         Caption         =   "วันที่ออก RV"
         Height          =   195
         Left            =   2880
         TabIndex        =   21
         Top             =   660
         Width           =   1035
      End
      Begin VB.Label Label5 
         BackColor       =   &H80000009&
         Caption         =   "วันที่รับเช็ค"
         Height          =   195
         Left            =   1440
         TabIndex        =   20
         Top             =   660
         Width           =   1035
      End
      Begin VB.Label Label3 
         BackColor       =   &H80000009&
         Caption         =   "รูปแบบรับเช็ค"
         Height          =   315
         Left            =   60
         TabIndex        =   18
         Top             =   2220
         Width           =   1095
      End
      Begin VB.Label Label2 
         BackColor       =   &H80000009&
         Caption         =   "เจ้าหน้าที่"
         Height          =   195
         Left            =   420
         TabIndex        =   16
         Top             =   1860
         Width           =   735
      End
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "วางบิลโดย"
         Height          =   195
         Index           =   0
         Left            =   360
         TabIndex        =   8
         Top             =   1020
         Width           =   720
      End
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "ลูกค้า"
         Height          =   195
         Index           =   4
         Left            =   660
         TabIndex        =   7
         Top             =   1500
         Width           =   375
      End
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "ช่วงวันที่"
         Height          =   195
         Index           =   3
         Left            =   420
         TabIndex        =   6
         Top             =   300
         Width           =   585
      End
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "ถึงวันที่"
         Height          =   195
         Index           =   2
         Left            =   3060
         TabIndex        =   5
         Top             =   300
         Width           =   495
      End
   End
   Begin VB.Label Label4 
      Caption         =   "รายการรับเช็ค FIN14"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
         Charset         =   222
         Weight          =   700
         Underline       =   0   'False
         Italic          =   0   'False
         Strikethrough   =   0   'False
      EndProperty
      ForeColor       =   &H8000000D&
      Height          =   315
      Left            =   240
      TabIndex        =   19
      Top             =   540
      Width           =   2955
   End
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Height          =   195
      Index           =   1
      Left            =   540
      TabIndex        =   15
      Top             =   2640
      Width           =   45
   End
   Begin VB.Image Image2 
      Height          =   435
      Index           =   1
      Left            =   0
      Picture         =   "frmPrintFIN14.frx":0059
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11280
   End
End
Attribute VB_Name = "frmPrintFIN14"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim curCustomer_ As BWGCustomer

Private Sub cmdClose_Click()
    Unload Me
End Sub

Private Sub cmdPreview_Click()
    PrintMain 0
End Sub

Private Sub cmdPrint_Click()
    PrintMain 1
End Sub

Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = 13 Then SendKeys "{Tab}"
End Sub

Private Sub Form_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then KeyAscii = 0
End Sub
'Private Sub txtSale_KeyDown(KeyCode As Integer, Shift As Integer)
'    If KeyCode = vbKeyReturn Then txtSale_Validate False
'End Sub
Private Sub Form_Load()
    CanAccess = False: CanUpdate = False
    SetPermission Me, CanAccess, CanUpdate
    If CanAccess = False Then
        MsgBox "ท่านไม่มีสิทธิใช้ระบบนี้", vbExclamation, "Warning"
        Unload Me
        Exit Sub
    End If
    
     With frmPicture
        cmdPreview.Picture = .picPreview.Picture
        cmdPrint.Picture = .picPrint.Picture
        cmdClose.Picture = .picExit.Picture
    End With
    PopulateCompany cboCompany, cboIndex
    PopulateStaffInfo cboRE_Men
End Sub

Private Sub PrintMain(printType As Integer)
Dim tmpSql As String
    With rpt1
        .LogonInfo(0) = "DSN = " & DSN_NAME & ";UID = " & strUid & ";PWD = " & strPwd
        .ReportFileName = App.Path & "\reports\FIN14.rpt"
        tmpSql = ""
        tmpSql = "SELECT * FROM vw_RptFIN01 " & vbCrLf
        If OptChek1.Value = True Then
            tmpSql = tmpSql & "WHERE PayDueDate>='" & dtFrom.ValueYMD & "' And PayDueDate<='" & dtTo.ValueYMD & "'"
        End If
        If OpChek2.Value = True Then
            tmpSql = tmpSql & "WHERE issuedDate >='" & dtFrom.ValueYMD & "' And issuedDate <='" & dtTo.ValueYMD & "'"
        End If
    
        If cboCompany.ListCount > 1 Then
            If cboCompany.ListIndex > 0 Then
                tmpSql = tmpSql & " AND CompanyID='" & CurrentUser.WorkCompanies(cboCompany.ListIndex + cboIndex).ID & "' "
            End If
        Else
            If cboCompany.ListIndex >= 0 Then
                tmpSql = tmpSql & " AND CompanyID='" & CurrentUser.WorkCompanies(cboCompany.ListIndex + cboIndex).ID & "' "
            End If
        End If
        tmpSql = tmpSql & " AND isCustAccepted='Y' and (isCancel<>'Y' or isCancel is null) "
        If Not curCustomer_ Is Nothing Then
            If Trim(curCustomer_.ID) <> "" Then
                tmpSql = tmpSql & " And CustomerID='" & curCustomer_.ID & "' "
            End If
        End If
         If cboRE_Men.ListIndex <> 0 Then
            tmpSql = tmpSql & " AND InfostaffNameRE = '" & cboRE_Men.Text & "' "
         End If
         If cboCheck.ListIndex = 1 Then
            tmpSql = tmpSql & " AND RE_State = 'C ' "
         End If
         If cboCheck.ListIndex = 2 Then
            tmpSql = tmpSql & " AND RE_State = 'B' "
         End If
         If cboCheck.ListIndex = 3 Then
            tmpSql = tmpSql & " AND RE_State = 'E' "
         End If
          If cboCheck.ListIndex = 4 Then
            tmpSql = tmpSql & " AND RE_State = 'S' "
         End If
        
'       colBillingInfoStaff_(cboRE_Men.ListIndex + 1).InFoStaffID
        
        tmpSql = tmpSql & vbCrLf & " ORDER BY PayDueDate"
        .Formulas(0) = "ShowDateRange=" & Chr(34) & dtFrom.ValueDMY & " ถึงวันที่ " & dtTo.ValueDMY & Chr(34)
        .Formulas(1) = "ShowPrintDate=" & Chr(34) & FormatYMD_to_DMY(TodayDate, "/", "/") & Chr(34)
        .Formulas(2) = "ShowCompany=" & Chr(34) & cboCompany.Text & Chr(34)
        .SQLQuery = tmpSql
        .Destination = printType
        .PrintReport
        If Trim(.LastErrorString) <> "" Then MsgBox .LastErrorString
    End With
End Sub

Private Sub txtCustomer_Validate(Cancel As Boolean)
    If curCustomer_ Is Nothing Then Set curCustomer_ = New BWGCustomer
    If Trim(txtCustomer.Text) = "" And Trim(curCustomer_.CustomerName) <> Trim(txtCustomer.Text) Then
        Set curCustomer_ = Nothing
        Set curCustomer_ = New BWGCustomer
    Else
        If Trim(curCustomer_.CustomerName) <> Trim(txtCustomer.Text) Then
            Set curCustomer_ = Nothing
            Set curCustomer_ = New BWGCustomer
            Load frmCustomerSearch
            With frmCustomerSearch
                .getCriteria (txtCustomer.Text)
                .SetCallerForm Me
                .Show 1
            End With
            If curCustomer_.ID = "" Then txtCustomer.Text = ""
        End If
    End If
End Sub

Public Sub SetDataCustomer(selCustomer As BWGCustomer, Optional isAssigned As Boolean = True)
Dim tmpRec As New ADODB.Recordset
Dim X%
Dim tmpWaste As BWGWasteDataCR
    Set curCustomer_ = Nothing
    Set curCustomer_ = selCustomer
    If curCustomer_ Is Nothing Then Exit Sub
    With curCustomer_
        txtCustomer.Text = .CustomerName
    End With
End Sub





