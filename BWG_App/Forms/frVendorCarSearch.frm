...
Begin VB.Form frmVendorCarSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F145"
   ClientHeight    =   6375
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   60
      Picture         =   "frVendorCarSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   5550
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาข้อมูลรถของ Vendor"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   18
         Charset         =   222
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "เลขทะเบียน, ประเภทรถ, ชื่อ Vendor"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   1740
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frVendorCarSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7020
   End
End
Attribute VB_Name = "frmVendorCarSearch"
...
Dim tmpItem As BWGVenderCar
    ...
    strCond = "(CarRegisID Like '%" & criStr & "%' Or SubTypeDesc Like '%" & criStr & "%' Or TruckTypeDesc Like '%" & criStr & "%' Or NameTH Like '%" & criStr & "%')"
    Set colSearchResult_ = tmpS.VenderCarSearch(strCond)
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.CarRegisID)
        tmpList.SubItems(1) = Trim(tmpItem.TruckTypeDesc & ", " & tmpItem.SubTypeDesc)
        tmpList.SubItems(2) = Trim(tmpItem.VendorName)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
