...
Begin VB.Form frmCompanySearch 
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   45
      Picture         =   "frCompanySearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   5505
      Width           =   915
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frCompanySearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7500
   End
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataCompany colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Private Sub cmdOpen_Click()
    If lvResult.ListItems.Count = 0 Then
        MsgBox "ไม่มีข้อมูลชื่อบริษัทที่ต้องการ", vbExclamation
        Exit Sub
    End If
    If lvResult.SelectedItem Is Nothing Then
        MsgBox "กรุณาเลือกข้อมูลชื่อบริษัทที่ต้องการ", vbExclamation
        Exit Sub
    End If
    callerForm_.SetDataCompany colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Dim tmpItem As BWGCompanyConfig
    ...
    strCond = "(CompanyCode Like '%" & criStr & "%' Or CompanyNameTH Like '%" & criStr & "%' Or CompanyNameEN Like '%" & criStr & "%')"
    Set colSearchResult_ = tmpS.CompanySearch(strCond)
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.CompanyCode)
        tmpList.SubItems(1) = Trim(tmpItem.CompanyNameTH)
        tmpList.SubItems(2) = Trim(tmpItem.CompanyNameEN)
        tmpList.SubItems(3) = Trim(tmpItem.DisposerNo)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
        If firstActive = True Then
'            result = MsgBox("ไม่พบข้อมูลที่ต้องการค้นหา ต้องการเพิ่มข้อมูลบริษัทใหม่หรือไม่", vbQuestion + vbYesNo)
            MsgBox "ไม่พบข้อมูลที่ต้องการค้นหา", vbInformation
'            If result = vbYes Then
'                frmCompany.cmdSearch.Enabled = False
'                frmCompany.Show 1
'                Set colSearchResult_ = tmpS.CompanySearch(strCond)
'                For x = 1 To colSearchResult_.Count
'                    Set tmpItem = colSearchResult_(x)
...
Dim tmpStr As String
    ...
    If KeyCode = 46 Then
        If MsgBox("ต้องการลบข้อมูลรายการนี้หรือไม่?", vbQuestion + vbYesNo) = vbYes Then
            Set tmpItem = New BWGCompanyConfig
