...
Dim tmpList As ListItem
    If KeyCode = 46 Then
        ...
        If tmpList Is Nothing Then
            ...
            Exit Sub
        End If
        frmUserList.DeleteUser Replace(tmpList.Key, "'", "")
        Set tmpList = Nothing
    End If
End Sub
...
Dim tmpS As New BWGSearchManager
    Set colUser_ = tmpS.UserSearch
    lvResult.ListItems.Clear
    For x = 1 To colUser_.Count
        Set tmpUser = colUser_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpUser.ID & "'", tmpUser.StaffFName & " " & tmpUser.StaffLName)
...
Dim tmpStr As String
    ...
    If Not CurrentUser Is Nothing Then
        If Trim(selID) = CurrentUser.ID Then
            MsgBox "ไม่สามารถลบข้อมูลพนักงานรายการนี้ได้ เพราะกำลังใช้งานอยู่", vbExclamation
            Exit Sub
        End If
    End If
    tmpKey = Trim("'" & selID & "'")
...
Public Sub SaveUser(selUser As BWGCompanyStaff)
Dim tmpKey As String
Dim tmpList As ListItem
    ...
    If isExist(colUser_, tmpKey) Then
        ...
        tmpList.Text = selUser.StaffFName & "  " & selUser.StaffLName
    Else
        Set tmpList = lvResult.ListItems.Add(, tmpKey, selUser.StaffFName & "  " & selUser.StaffLName)
    End If
...
Public Sub EditUser()
Dim tmpUser As BWGCompanyStaff
    If frmUserList.lvResult.SelectedItem Is Nothing Then
        MsgBox "กรุณาเลือกรายการที่ต้องการแก้ไขข้อมูล", vbExclamation
        Exit Sub
    End If
    Set tmpUser = frmUserList.getUserList(frmUserList.lvResult.SelectedItem.Key)
    Load frmUserProfile
    With frmUserProfile
...
Public Sub UpdateItem(selItem As BWGCompanyStaff)
Dim tmpKey As String
Dim tmpList As ListItem
    With selItem
        tmpKey = Trim("'" & .ID & "'")
        If isExist(colUser_, tmpKey) Then
            Set tmpList = lvResult.ListItems(tmpKey)
            tmpList.Text = .StaffFName & " " & .StaffLName
            colUser_.Remove tmpKey
        Else
            Set tmpList = lvResult.ListItems.Add(, "'" & .ID & "'", .StaffFName & " " & .StaffLName)
        End If
        If .isActive Then
            tmpList.SubItems(1) = "พร้อมใช้งาน"
        Else
            tmpList.SubItems(1) = "ระงับการใช้งาน"
        End If
        If colUser_ Is Nothing Then Set colUser_ = New Collection
