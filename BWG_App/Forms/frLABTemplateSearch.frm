...
Begin VB.Form frmLABTemplateSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F051"
   ClientHeight    =   6375
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   60
      Picture         =   "frLABTemplateSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   5550
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาข้อมูล LABTemplate"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   18
         Charset         =   222
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frLABTemplateSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7020
   End
End
Attribute VB_Name = "frmLABTemplateSearch"
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataLABTemplate colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataLABTemplate colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Dim tmpItem As BWGLABTemplateHeader
    ...
    strCond = "(TemplateName Like '%" & criStr & "%')"
    Set colSearchResult_ = tmpS.LABTemplateHeaderSearch(strCond)
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.TemplateNo)
        tmpList.SubItems(1) = Trim(tmpItem.TemplateName)
        tmpList.SubItems(2) = Trim(tmpItem.TemplateName & " " & tmpItem.TestTypeName)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
        If firstActive = True Then
            result = MsgBox("ไม่พบข้อมูลที่ต้องการค้นหา ต้องการเพิ่มข้อมูลผู้ประกอบการขนส่งใหม่หรือไม่", vbQuestion + vbYesNo)
            If result = vbYes Then
                frmLABTemplateSearch.cmdSearch.Enabled = False
                frmLABTemplateSearch.Show 1
                Set colSearchResult_ = tmpS.LABTemplateHeaderSearch(strCond)
                For x = 1 To colSearchResult_.Count
                    Set tmpItem = colSearchResult_(x)
                    Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.TemplateNo)
