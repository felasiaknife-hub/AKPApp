...
Begin VB.Form frmTransportVenderSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F129"
   ClientHeight    =   6375
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   60
      Picture         =   "frTransportVenderSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   5550
      Width           =   915
   ...
   Begin VB.Label AumphurSearch 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ค้นหาข้อมูลผู้ประกอบการขนส่งร่วม"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   18
         Charset         =   222
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frTransportVenderSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7020
   End
End
Attribute VB_Name = "frmTransportVenderSearch"
...
Dim VendorType_ As String

Public Sub VendorType(Value As String)
    VendorType_ = Value
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataTransportVender colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataTransportVender colSearchResult_(lvResult.SelectedItem.Key)
    Me.Hide
End Sub
...
Dim tmpItem As BWGTransportVendor
    ...
    If Trim(VendorType_) <> "" Then
        strCond = "(NameTH Like '%" & criStr & "%' AND VendorType = '" & VendorType_ & "')"
    Else
        strCond = "(NameTH Like '%" & criStr & "%' AND VendorType = 'VC')"
    End If
    ...
    If lvResult.ListItems.Count = 0 Then
        If firstActive = True Then
            ...
            If result = vbYes Then
                frmTransportVenderSearch.cmdSearch.Enabled = False
                'frmTransportVenderSearch.Show
                frmTransportVenderSearch.ZOrder
                Set colSearchResult_ = tmpS.TransportVenderSearch(strCond)
                For x = 1 To colSearchResult_.Count
                    Set tmpItem = colSearchResult_(x)
                    Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.NameTH)
