...
Begin VB.Form frmAdjustDeptSearch 
   BackColor       =   &H00FFFFFF&
   Caption         =   "F011"
   ClientHeight    =   6735
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   30
      Picture         =   "frAdjustDeptSearch.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   4
      Top             =   5910
      Width           =   915
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "เลขที่เอกสาร, ชื่อลูกค้า"
      ForeColor       =   &H000000FF&
      Height          =   195
      Index           =   2
      Left            =   2130
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frAdjustDeptSearch.frx":27A2
      Stretch         =   -1  'True
      Top             =   0
      Width           =   9150
   End
End
Attribute VB_Name = "frmAdjustDeptSearch"
...
Private Sub cmdOpen_Click()
    If lvResult.ListItems.Count = 0 Then
        MsgBox "ไม่มีข้อมูลใบเพิ่ม/ลด หนี้ ที่ต้องการ", vbExclamation
        Exit Sub
    End If
    If lvResult.SelectedItem Is Nothing Then
        MsgBox "กรุณาเลือกข้อมูลใบเพิ่ม/ลด หนี้ ที่ต้องการ", vbExclamation
        Exit Sub
    End If
    callerForm_.SetDataAdjustDept colSearchResult_(lvResult.SelectedItem.Key)
    Me.Hide
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataAdjustDept colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpItem As BWGAdjDeptHeader
    ...
    strCond = "(AdjustBillNo Like '%" & criStr & "%') OR (CustomerName Like '%" & criStr & "%'))"
    ...
    Set colSearchResult_ = tmpS.AdjustDeptHeaderSearch(strCond)
    Set tmpS = Nothing
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.AdjustBillNo)
        If tmpItem.AdjustType = "A" Then
            tmpList.SubItems(1) = "ใบลดหนี้ (ไม่ใช่ใบกำกับ)"
        ElseIf tmpItem.AdjustType = "B" Then
            tmpList.SubItems(1) = "ใบลดหนี้ / ใบกำกับภาษี"
        ElseIf tmpItem.AdjustType = "C" Then
            tmpList.SubItems(1) = "ใบเพิ่มหนี้ (ไม่ใช่ใบกำกับ)"
        ElseIf tmpItem.AdjustType = "D" Then
            tmpList.SubItems(1) = "ใบเพิ่มหนี้ / ใบกำกับภาษี"
        End If
        tmpList.SubItems(2) = FormatYMD_to_DMY(Trim(tmpItem.IssuedDate), "/", "/")
