...
Begin VB.Form frmPrintENV03 
   ...
   Begin VB.Label Label1 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "พิมพ์รายงานจำนวน Waste ที่ส่งวิเคราะห์ (ENV03)"
      BeginProperty Font 
         Name            =   "Angsana New"
         Size            =   15.75
         Charset         =   222
   ...
   Begin VB.Image Image2 
      ...
      Index           =   1
      Left            =   0
      Picture         =   "frPrintENV03.frx":0000
      Stretch         =   -1  'True
      Top             =   0
      Width           =   11280
   End
End
Attribute VB_Name = "frmPrintENV03"
...
Dim curLabVendor_ As BWGTransportVendor

Private Sub cboLAB_Click()
    If cboLAB.ListIndex = 0 Or cboLAB.ListIndex = 2 Then
        txtLabVendor.Visible = True
    Else
        txtLabVendor.Visible = False
        Set curLabVendor_ = New BWGTransportVendor
        txtLabVendor.Text = ""
    End If
End Sub
...
Dim tmpSql As String
    With rpt1
        ...
        .ReportFileName = App.Path & "\reports\ENV03.rpt"
        tmpSql = ""
        tmpSql = "SELECT * FROM vw_RptENV03 " & vbCrLf
        tmpSql = tmpSql & "WHERE SendDate>='" & dtFrom.ValueYMD & "' And SendDate<='" & dtTo.ValueYMD & "' "
        ...
        Else
            ...
            End If
        End If
        If cboLAB.ListIndex = 1 Then
            tmpSql = tmpSql & " And isTestByExtVendor='N' "
        ElseIf cboLAB.ListIndex = 2 Then
            If Trim(txtLabVendor.Text) = "" Then
                tmpSql = tmpSql & " And (isTestByExtVendor<>'N' Or isTestByExtVendor is null) "
            Else
                tmpSql = tmpSql & " And (isTestByExtVendor<>'N' Or isTestByExtVendor is null) And LabVendorID='" & curLabVendor_.ID & "' "
            End If
        End If
        tmpSql = tmpSql & vbCrLf & " ORDER BY SendDate,NameTH"
        .Formulas(0) = "DateRange=" & Chr(34) & "ช่วงวันที่ส่ง " & dtFrom.ValueDMY & " ถึงวันที่ " & dtTo.ValueDMY & Chr(34)
...
Private Sub txtLabVendor_Validate(Cancel As Boolean)
    If curLabVendor_ Is Nothing Then Set curLabVendor_ = New BWGTransportVendor
    If Trim(txtLabVendor.Text) = "" And Trim(curLabVendor_.NameTH) <> Trim(txtLabVendor.Text) Then
        Set curLabVendor_ = Nothing
        Set curLabVendor_ = New BWGTransportVendor
    Else
        If Trim(curLabVendor_.NameTH) <> Trim(txtLabVendor.Text) Then
            Set curLabVendor_ = Nothing
            Set curLabVendor_ = New BWGTransportVendor
            Load frmTransportVenderSearch
            With frmTransportVenderSearch
                .getCriteria (txtLabVendor.Text)
                .SetCallerForm Me
                .Show 1
            End With
            If curLabVendor_.ID = "" Then txtLabVendor.Text = ""
        End If
    End If
End Sub
...
Dim x%, hasCar As Boolean
    Set curLabVendor_ = selVendor
    With curLabVendor_
        txtLabVendor.Text = .NameTH
    End With
End Sub
