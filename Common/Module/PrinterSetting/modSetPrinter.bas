Attribute VB_Name = "modSetPrinter"
Option Explicit
...
Public InvPaperLayout As Integer
Public RecPaperLayout As Integer
Public BowPaperLayout As Integer
...
Public Sub ChngPrinterOrientationPortrait(ByVal frm As Form)
   ...
   Call SetOrientation(DMDUP_SIMPLEX, PageDirection, frm)
End Sub
Public Sub ResetPrinterPaperSize(ByVal frm As Form)
   ...
   Else
       myPapersize = 1
   End If
   Call SetPapersize(DMDUP_SIMPLEX, myPapersize, frm)
End Sub
Public Sub ChngPrinterPaperSizeLetter(ByVal frm As Form)

   myPapersize = 1
   Call SetPapersize(DMDUP_SIMPLEX, myPapersize, frm)
End Sub
Public Sub ChngPrinterPaperSizeLedger(ByVal frm As Form)
...
Dim tmpVal As String
    ...
    If Right(configFile, 1) = "\" Then
        configFile = configFile & "SpecConfig.txt"
    Else
        configFile = configFile & "\SpecConfig.txt"
    End If
    ...
    Do Until tmpTxt.AtEndOfStream
        ...
        If InStr(1, tmpStr, "=") > 0 Then
            ...
            If IsNumeric(tmpVal) Then
                If InStr(1, tmpStr, "INVFORM_ORIENTATION") > 0 Then
                    InvPaperLayout = CInt(tmpVal)
                ElseIf InStr(1, tmpStr, "RECFORM_ORIENTATION") > 0 Then
                    RecPaperLayout = CInt(tmpVal)
                ElseIf InStr(1, tmpStr, "BOWFORM_ORIENTATION") > 0 Then
                    BowPaperLayout = CInt(tmpVal)
                End If
            Else
                If InStr(1, tmpStr, "REPORT_DSN") > 0 And Trim(tmpVal) <> "" Then
                    'DSN_NAME = Trim(tmpVal)
                ElseIf InStr(1, tmpStr, "DB_PATH") > 0 And Trim(tmpVal) <> "" Then
                    MDB_PATH = Trim(tmpVal)
                ElseIf InStr(1, tmpStr, "ServerName") > 0 And Trim(tmpVal) <> "" Then
                    ServerName = Trim(tmpVal)
                ElseIf InStr(1, tmpStr, "UserID") > 0 And Trim(tmpVal) <> "" Then
                    SQLUserID = Trim(tmpVal)
                ElseIf InStr(1, tmpStr, "Password") > 0 And Trim(tmpVal) <> "" Then
                    SQLPassWord = Trim(tmpVal)
                End If
            End If
        End If
    Loop
