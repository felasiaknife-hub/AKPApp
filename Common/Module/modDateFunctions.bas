...
Public Function GetDateServer() As String
    ...
    Dim tSql As String
    tSql = "SELECT CONVERT(DATETIME, GETDATE(), 103) AS GETDATE"
    Set RS = GetRS(tSql, adOpenForwardOnly, adLockReadOnly)
    GetDateServer = RS!GetDate
End Function
...
Public Function GetLastDateofMonth(ByVal dCurrDate As Date) As String
  Dim dFirstDayNextMonth As Date
  
  On Error GoTo lbl_Error
 
  GetLastDateofMonth = Empty
  dFirstDayNextMonth = DateSerial(CInt(Format(dCurrDate, "yyyy")) - 543, CInt(Format(dCurrDate, "mm")) + 1, 1)
  GetLastDateofMonth = Format(DateAdd("d", -1, dFirstDayNextMonth), "yyyy/MM/dd")
...
Public Function GetLastDateofPrevMonth(ByVal dCurrDate As Date) As String
  Dim dFirstDayNextMonth As Date
  
  On Error GoTo lbl_Error
 
  GetLastDateofPrevMonth = Empty
  dFirstDayNextMonth = DateSerial(CInt(Format(dCurrDate, "yyyy")) - 543, CInt(Format(dCurrDate, "mm")), 1)
  GetLastDateofPrevMonth = Format(DateAdd("d", -1, dFirstDayNextMonth), "yyyy/MM/dd")
