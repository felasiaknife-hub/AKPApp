...
Begin VB.Form frUserGroup 
   ...
   Begin VB.Image Image2 
      Height          =   915
      Left            =   0
      Picture         =   "frUserGroup.frx":13C6
      Stretch         =   -1  'True
      Top             =   0
      Width           =   10050
   End
End
Attribute VB_Name = "frUserGroup"
...
Dim tmpList As listItem
    ...
    If myGroup_.isChanged Then
        myGroup_.Save
        If isExist(lvResult.ListItems, "'" & myGroup_.ID & "'") Then
            Set tmpList = lvResult.ListItems("'" & myGroup_.ID & "'")
        Else
            Set tmpList = lvResult.ListItems.Add(, "'" & myGroup_.ID & "'", myGroup_.Name)
            ColUserGroup.Add myGroup_, "'" & myGroup_.ID & "'"
        End If
        tmpList.SubItems(1) = myGroup_.Description
...
Private Sub lvResult_ItemClick(ByVal Item As MSComctlLib.listItem)
    ...
    If "'" & myGroup_.ID & "'" <> Trim(Item.Key) Then
        cmdNew_Click
    End If
    Set myGroup_ = Nothing
    Set myGroup_ = ColUserGroup(Item.Key)
