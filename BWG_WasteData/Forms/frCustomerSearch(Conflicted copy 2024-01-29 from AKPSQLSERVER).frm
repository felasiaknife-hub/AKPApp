...
Begin VB.Form frmCustomerSearch 
   ...
   Begin VB.Label Label2 
      AutoSize        =   -1  'True
      BackStyle       =   0  'Transparent
      Caption         =   "ชื่อ, ชื่อกลุ่มลูกค้า, เลขทะเบียนโรงงาน"
      ForeColor       =   &H00000000&
      Height          =   195
      Index           =   2
      Left            =   1740
...
Dim tmpItem As BWGCustomer
    ...
    strCond = "(CompanyName Like '%" & criStr & "%' Or CustomerCode Like '%" & criStr & "%' Or RegisterNo Like '%" & criStr & "%') "
    If callerForm_.Name = "frmCustomer" Then
        If frmCustomer.CallLocation = "P" Then strCond = "(" & strCond & " And CustTypeFlag  in ('C','L','T','G','B') )"
    End If
    
    
    If isAkp = True And isPST = True And isBWG = False And isBWC = False Then
        strCond = strCond & " And (SDName like 'AP%' Or SDName like 'RAP%' Or SDName like 'ME%')"
    ElseIf isAkp = False And isBWG = False And isPST = True And isBWC = True Then
        strCond = strCond & " And SDName like '%BWC%' Or SDName like 'ME%'"
    ElseIf isPST = True And isBWG = False And isBWC = False And isAkp = False Then
        strCond = strCond & " And SDName like '%BWC%' Or SDName like 'ME%'"
    End If

    
    strCond = strCond & " And (Status = 'N' Or Status = 'P' Or Status = 'R'  Or Status = 'D' Or Status = 'J' Or Status = 'L' Or Status = 'P' )"
