...
Begin VB.Form frmTumbolSearch 
   Caption         =   $"frTumbolSearch.frx":0000
   ClientHeight    =   6405
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   7290
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   60
      Picture         =   "frTumbolSearch.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   0
      Top             =   5580
      Width           =   915
   ...
   Begin VB.Image Image2 
      Height          =   705
      Left            =   0
      Picture         =   "frTumbolSearch.frx":27AE
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7320
   End
   ...
   End
End
Attribute VB_Name = "frmTumbolSearch"
...
Private Sub cmdOpen_Click()
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataTumbol colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataTumbol colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Dim tmpItem As IMITumbol
    ...
    strCond = "(TumbolName Like '%" & criStr & "%')"
    
    Set colSearchResult_ = tmpS.TumbolSearch(strCond)
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.Name)
        tmpList.SubItems(1) = Trim(tmpItem.AumphurName)
        tmpList.SubItems(2) = Trim(tmpItem.ProvinceName)
        tmpList.SubItems(3) = Trim(tmpItem.ADesc)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
    If lvResult.ListItems.Count = 0 Then
        result = MsgBox("ไม่พบข้อมูลที่ต้องการค้นหา ต้องการเพิ่มข้อมูลตำบลใหม่หรือไม่", vbQuestion + vbYesNo)
