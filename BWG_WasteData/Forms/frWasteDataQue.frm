...
Dim sqlStr As String, sqlStr1 As String, sqlStr2 As String, sqlStr3 As String, strCond As String, statusDesc As String, sqlGroup As String
Dim tmpArr
...
NewCount = 0
    ...
    sqlStr = sqlStr & "ISNULL(isNew, 'N') AS isNew, SaleStaff, ISNULL(CAST(RejectDetail AS NVARCHAR(MAX)), 'N') AS RejectDetail, ISNULL(CompanyCode, '') AS CompanyCode, " & vbCrLf
...
'    End If
    ...
    If Option5.Value = False Then
        ...
        End If
    End If
    sqlGroup = " group by  WasteDataID,SaleSendDateAuto,AcceptedDate,WasteNo,WasteName,Customercode,CompanyName,EUCode1,EUCode2,EUCode3,isApproveByLab,WasteType ,isTreatAble,isCancel"
    sqlGroup = sqlGroup & " ,LicenseExpDate,KorOrNumber,TreatmentDesc,isLabResultNeeded, isRequest"
    sqlGroup = sqlGroup & " , isAcceptedByEnvi, isNew, SaleStaff, CAST(RejectDetail AS NVARCHAR(MAX)),CompanyCode,RequestDate, Status, isQuotationReq"
    sqlGroup = sqlGroup & " , isApprove, CisNew, CisRequest, RejectDesc,WasteYears,CStatus,LBstatus,CreatedDate,CaseName,ReceivedDate, AnalyticalDate, LBStatus, isApproveLab,WasteReferrent"
...
'            End If
            ...
            Set tmpRec = GetRS(sqlStr & sqlStr1 & strCond & sqlGroup & " Order By WasteNo", adOpenForwardOnly, adLockReadOnly)
