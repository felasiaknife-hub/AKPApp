...
Begin VB.Form frQuotation 
   ...
   Begin VB.Frame Frame3 
      ...
      Top             =   5550
      Width           =   11385
      Begin WasteDataForm.ctlGrid grRemark 
         Height          =   2235
         Left            =   0
         TabIndex        =   80
         Top             =   120
   ...
   Begin VB.Frame Frame1 
      ...
      Top             =   690
      Width           =   11385
      Begin WasteDataForm.ctlDate dtIssuedDate 
         Height          =   315
         Left            =   4440
         TabIndex        =   81
         Top             =   240
      ...
      Begin VB.ComboBox cboIsTrailer 
         Enabled         =   0   'False
         Height          =   315
         ItemData        =   "frQuotation_AP.frx":0000
         Left            =   3360
         List            =   "frQuotation_AP.frx":000A
         Style           =   2  'Dropdown List
         TabIndex        =   11
         Top             =   1440
         Width           =   1215
   ...
   Begin VB.Frame Frame2 
      ...
      Top             =   3030
      Width           =   11385
      Begin WasteDataForm.ctlGrid grItem 
         Height          =   2235
         Left            =   60
         TabIndex        =   79
         Top             =   240
   ...
   Begin VB.Image Image2 
      Height          =   645
      Left            =   0
      Picture         =   "frQuotation_AP.frx":0020
      Stretch         =   -1  'True
      Top             =   0
      Width           =   12615
   End
End
Attribute VB_Name = "frQuotation"
...
Dim tmpStr As String
    tmpStr = CurrentUser.VerifyAccessRole(frWasteDataAdd.Name, AccessRole)
    If Trim(tmpStr) <> "" Then
    ...
    If result = vbYes Then
        If CheckPendingJob Then
            If MsgBox("มี Job ที่ยังไม่ได้ออก Manifest ใช้งานใบเสนอราคานี้อยู่ ท่านต้องการย้าย Job เหล่านี้ไปใช้งานใบเสนอราคาอื่นหรือไม่", vbQuestion + vbYesNo) = vbYes Then
                Load frQuotationTransfer2
                With frQuotationTransfer2
                    .SetDataQuotation myQuotation_
                    .SetPendingJob colPendingJob_, txtQuoNo.Text
                    .Show 1
                End With
            
'                Load frQuotationTransfer
'                With frQuotationTransfer
'                    .SetPendingJob colPendingJob_, txtQuoNo.Text
'                    .Show 1
'                End With
                If CheckPendingJob Then
...
Dim tmpCol As Collection
    If myQuotation_.isChanged And cmdSave.Enabled Then
        ...
        End If
    End If
    Load frQuotationSearch
    With frQuotationSearch
        .SetCallerForm Me
        .Show 1
    End With
End Sub
...
Dim tmpStr As String
    tmpStr = CurrentUser.VerifyAccessRole(frRemDesc.Name, AccessRole)
    If Trim(tmpStr) <> "" Then
        MsgBox tmpStr, vbExclamation
        Exit Sub
    End If
...
'    grItem.ColInputType(5) = ComboBoxEnum
    ...
    With grItem.getGridObj
        ...
        .TextMatrix(0, 2) = "อัตราค่าบำบัด"
        .TextMatrix(0, 3) = "หน่วย"
        .TextMatrix(0, 4) = "ค่า Blending"
        .TextMatrix(0, 5) = "ค่าเกินเกณฑ์การออกแบบ"
        .TextMatrix(0, 6) = "รวมค่ากำจัด"
    End With
...
Private Sub grItem_BeforeUpdateValue(newValue As String, row As Integer, col As Integer, Cancel As Boolean)
    If col = 2 Or col = 4 Or col = 5 Then
        If newValue <> "" Then
...
Set myQuotation_ = New BWGQuotationHeader
    ...
    With myQuotation_
        .ID = GetGUID
        If cboCompany.ListIndex > 0 Then
            .companyID = CurrentUser.WorkCompanies(cboCompany.ListIndex).ID
        End If
...
Dim HeatValue As Double
    With grItem.getGridObj
        ...
        For x = 1 To tmpCol.Count
            ...
            .TextMatrix(x + rowCount, 2) = tmpDet.BurningRate
            .TextMatrix(x + rowCount, 3) = tmpDet.TreatmentUnitName
            .TextMatrix(x + rowCount, 4) = tmpDet.BlendingRate
            .TextMatrix(x + rowCount, 5) = tmpDet.ParameterRate
            .TextMatrix(x + rowCount, 6) = tmpDet.BurningRate + tmpDet.BlendingRate + tmpDet.ParameterRate
            tmpDet.RowPos = x
        Next
        .row = 1
        .col = 1
...
Dim x%
    cboCompany.Clear
    cboCompany.AddItem "บริษัทที่ออกใบเสนอราคา"
