...
Begin VB.Form frmEUCode2Search 
   BackColor       =   &H00FFFFFF&
   Caption         =   $"frEUCode2Search.frx":0000
   ClientHeight    =   6375
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   7005
   ...
   Begin VB.CommandButton cmdOpen 
      ...
      Height          =   795
      Left            =   60
      Picture         =   "frEUCode2Search.frx":000C
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   5550
      Width           =   915
   ...
   Begin VB.Image Image2 
      Height          =   795
      Left            =   -15
      Picture         =   "frEUCode2Search.frx":27AE
      Stretch         =   -1  'True
      Top             =   0
      Width           =   7020
   End
End
Attribute VB_Name = "frmEUCode2Search"
...
Private Sub Form_Activate()
    If firstActive Then
        ...
        If lvResult.ListItems.Count = 1 Then
            callerForm_.SetDataEUCode2 colSearchResult_(1)
            Unload Me
        End If
    End If
End Sub
...
Private Sub cmdOpen_Click()
    ...
    If lvResult.SelectedItem Is Nothing Then
        ...
        Exit Sub
    End If
    callerForm_.SetDataEUCode2 colSearchResult_(lvResult.SelectedItem.Key)
    Unload Me
End Sub
...
Dim tmpItem As BWGEUCode2
    ...
    strCond = "((Code2 Like '%" & criStr & "%' Or CodeDesc Like '%" & criStr & "%') And EUCode1ID='" & callerForm_.getEUCode1.ID & "')"
    Set colSearchResult_ = tmpS.EUCode2Search(strCond)
    For x = 1 To colSearchResult_.Count
        Set tmpItem = colSearchResult_(x)
        Set tmpList = lvResult.ListItems.Add(, "'" & tmpItem.ID & "'", tmpItem.Code2)
