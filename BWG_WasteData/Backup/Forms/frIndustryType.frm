...
Begin VB.Form frmIndustryType 
   ...
   Caption         =   $"frIndustryType.frx":0000
   ClientHeight    =   6045
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   6840
   ...
   Begin VB.Image Image2 
      Height          =   705
      Left            =   0
      Picture         =   "frIndustryType.frx":000C
      Stretch         =   -1  'True
      Top             =   0
      Width           =   6870
   End
...
Dim aa
    If cmdSave.Enabled = True Then
        If Trim(txtShortName.Text) <> Trim(curIndustryType_.TypeDesc) Then
...
Dim tmpStr As String
    ...
    If Trim(txtShortName.Text) <> Trim(curIndustryType_.TypeDesc) Then
        ...
        If Trim(curIndustryType_.TypeID) = "" Then
            curIndustryType_.TypeID = GetGUID
            tmpCon.Execute "Insert into tbIndustryType_New (Typecode,TypeDesc) Values ('" & Trim(curIndustryType_.TypeID) & "','" & (txtShortName.Text) & "')"
            Set tmpList = lvList.ListItems.Add(, "'" & curIndustryType_.TypeID & "'", txtShortName.Text)
            curIndustryType_.TypeDesc = Trim(txtShortName.Text)
            colIndustryType_.Add curIndustryType_, "'" & curIndustryType_.TypeID & "'"
        Else
            tmpCon.Execute "Update tbIndustryType_New Set TypeDesc='" & Trim(txtShortName.Text) & "' Where TypeID='" & curIndustryType_.TypeID & "'"
            Set tmpList = lvList.ListItems("'" & curIndustryType_.TypeID & "'")
            curIndustryType_.TypeDesc = Trim(txtShortName.Text)
        End If
...
Dim x%
Dim tmpList As ListItem
Dim tmpItem As BWGIndustryType_New
    If colIndustryType_ Is Nothing Then Exit Sub
    lvList.ListItems.Clear
    For x = 1 To colIndustryType_.Count
        Set tmpItem = colIndustryType_(x)
        Set tmpList = lvList.ListItems.Add(, "'" & tmpItem.TypeID & "'", tmpItem.TypeCode & " - " & tmpItem.TypeDesc)
        Set tmpItem = Nothing
        Set tmpList = Nothing
    Next
End Sub
...
'        txtShortName.Text = curIndustryType_.TypeDesc
'        curRole_ = UpdateRole
'    End If
End Sub
...
On Error Resume Next
    ...
    strCond = "TypeCode like '%" & Trim(txtShortName.Text) & "%' or typedesc like '%" & Trim(txtShortName.Text) & "%' "
    Set colSearchResult_ = tmpS.IndustryTypeSearch(strCond)
