...
Private Sub DumpXMLNode(node As MSXML.IXMLDOMNode, lvl%)
    
    ' count children
    Dim i%, iChildren%
    iChildren = node.childNodes.length
    
    ' single child? add leaf node
    If iChildren = 1 Then
        fg.AddItem node.nodeName & vbTab & node.Text
    ...
    fg.AddItem node.nodeName
    fg.IsSubtotal(fg.Rows - 1) = True
    fg.RowOutlineLevel(fg.Rows - 1) = lvl
    
    ' dump child nodes
    For i = 0 To iChildren - 1
        DumpXMLNode node.childNodes(i), lvl + 1
    Next
