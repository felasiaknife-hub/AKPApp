...
// Attributes
	...
	HICON SetIcon(UINT nID)
	{
		HICON hIcon = LoadIcon(_Module.GetResourceInstance(), MAKEINTRESOURCE(nID));
		HICON rv = SetIcon(hIcon);
		DestroyIcon(hIcon);
		return rv;
	}
	HICON SetIcon(HICON hIcon)
...
// Operations
	...
	{
		TCHAR buff[256];
		LoadString(_Module.GetResourceInstance(), nID, buff, 256);
...
// Operations
	...
	int Add(UINT iID, COLORREF crMask = RGB(255,255,255))
	{
		ATLASSERT(m_hImageList != NULL);
		return ImageList_AddMasked(m_hImageList, 
			LoadBitmap(_Module.GetResourceInstance(), MAKEINTRESOURCE(iID)), crMask);
	}
...
// Operations
	...
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return (DWORD)::SendMessage(m_hWnd, TB_SETEXTENDEDSTYLE, 0, (LPARAM)dwStyle);
	}
	...
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return (BOOL)::SendMessage(m_hWnd, TB_GETBUTTONINFO, (WPARAM)nID, (LPARAM)lptbbi);
	}
	...
	BOOL SetButtonCx(int nID, int iWidth)
	{
		TBBUTTONINFO bi;
		bi.cbSize = sizeof(TBBUTTONINFO);
		bi.dwMask = TBIF_SIZE;
		bi.cx = iWidth;
		return SetButtonInfo(nID,  &bi);
	}
	BOOL SetButtonState(int nID, UINT iState)
	{
		TBBUTTONINFO bi;
		bi.cbSize = sizeof(TBBUTTONINFO);
	...
	UINT GetButtonState(int nID)
	{
		TBBUTTONINFO bi;
		bi.cbSize = sizeof(TBBUTTONINFO);
		bi.dwMask = TBIF_STATE;
		GetButtonInfo(nID, &bi);
		return bi.fsState;
	}
	HIMAGELIST GetImageList()
	{
...
// Operations
	...
	BOOL InsertItem(int nItem, int nID)
	{
		TCHAR buff[256];
		LoadString(_Module.GetResourceInstance(), nID, buff, 256);
		TC_ITEM tci;
