...
Dim m, d As Integer
    ...
    If Not CurrentUser Is Nothing Then
        Unload Me
        If CheckUserPermission(txtUserName) = False Then
            MsgBox "ท่านไม่มีสิทธิใช้งานโปรแกรมนี้", vbInformation, "สิทธิการใช้งานโปรแกรม"
            End
        Else
            frmMDIComm.Show
        End If
...
'            tmpRec.Open "Select * from tbCompanyStaff Where LoginName='" & Trim(txtUserName.Text) & "'", DBConn, adOpenKeyset, adLockOptimistic
    ...
    ElseIf KeyCode = 116 Then
        If Trim(txtUserName.Text) <> "" And Trim(txtPassword.Text) <> "" Then
            DBConnExc "Update tbTeacherData Set LoginPwd='" & encObj.EncryptString(Trim("" & txtPassword.Text), ENC_KEY) & "' Where LoginName='" & Trim(txtUserName.Text) & "'"
        End If
    End If
    Set tmpRec = Nothing
    Set encObj = Nothing
